...
public class RecipeSelect extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recipeRecycler = (RecyclerView) findViewById(R.id.recipes);

        nAdapter = new RecipeAdapter(recipeList);
        RecyclerView.LayoutManager mLayoutManager =
                new LinearLayoutManager(getApplicationContext());
        recipeRecycler.setLayoutManager(mLayoutManager);
        recipeRecycler.setItemAnimator(new DefaultItemAnimator());
        recipeRecycler.setAdapter(nAdapter);
    ...
    private void prepareRecipes() {
        ...
        for (int f = 1; f <= files.length; f++) {
            theFile = "Recipe" + f + ".txt";
            RecipeBuilder recipe = new RecipeBuilder(theFile, Open(theFile));
